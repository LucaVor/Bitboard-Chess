...
public class Fracture : MonoBehaviour
{
    ...
    public static GameObject PermittedShatter = null;
    ...
    {
        if (triggerOptions.triggerType == TriggerType.Trigger && PermittedShatter == transform.parent.gameObject)
        {
            // Colliding object tag must be in the set of allowed collision tags if filtering by tag is enabled
    ...
    private void ComputeFracture()
    {
        ...
        if (mesh != null)
        {
            ...
            else
            {
                ...
                Debug.Log (fragmentRoot.transform.childCount);
                foreach (Transform child in fragmentRoot.transform)
                {
                    Rigidbody rb = child.GetComponent<Rigidbody>();
                    rb.useGravity = true;
                    rb.AddForce (
                        (UnityEngine.Random.insideUnitSphere * 30) + Vector3.up * 10
                    );
