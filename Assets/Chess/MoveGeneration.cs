...
namespace QuickChess
{
    ...
    public class MoveGeneration
    {
        ...
        public UInt64[] GenerateMoves(bool psuedo = false)
        {
            ...
            for (int pc = 0; pc < 6; pc ++)
            {
                ...
                for (int index = 0; index < piece.pieces; index ++)
                {
                    ...
                    {
                        case 0:                              
                            move = GetKingMoves (i, colour, friendly, enemy, inCheck);
                            move = MakeLegal (
            ...
            if (dir != Direction.None)
            {
                ...
                if (PreProcessing.Diagonals[(int)dir] == 0) {
                    ...
                    if (spotRookMask != 0) {
                        ...
                        {
                            pinnerIndex = _ForwardBitScan (spotRookMask);
                        } else {
                ...
                if (PreProcessing.Diagonals[(int)dir] == 1) {
                    ...
                    if (spotBishopMask != 0) {
                        ...
                        if (PreProcessing.Upwards[(int)dir] == 1)
                        {
                            pinnerIndex = _ForwardBitScan (spotBishopMask);
                        } else {
        ...
        public EnemyAttackingReturnData GetAllAttackedSquaresExcludingKing (int colour, UInt64 friend, UInt64 enemy, Pieces[] friendlyPieces, int targetKing)
        {
            ...
            for (int pc = 0; pc < 6; pc ++)
            {
                ...
                for (int index = 0; index < piece.pieces; index ++)
                {
                    ...
                    {
                        case 0:    
                            // We are just saying "True" to in check to exclude all castling options                          
                            move = GetKingMoves (i, colour, enemy, friend, true, EXCLUDE_KING);
        ...
        public UInt64 GetKingMoves (int square, int colour, UInt64 friend, UInt64 enemy, bool isInCheck, int useKingMask = INCLUDE_KING)
        {
            ...
            if (canCastle && castlingMoves != 0 && !isInCheck)
            {
        ...
        public UInt64 GetBishopMoves(int square, UInt64 friend, UInt64 enemy, int useKingMask = INCLUDE_KING)
        {
            ...
            {
                int blockerIndex = _ForwardBitScan (
